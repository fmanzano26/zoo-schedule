  ...
  return (
    <form onSubmit={save} className="space-y-5 text-gray-100">
      ...
      <div className="flex items-center justify-end gap-3">
        <button type="button" className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-neutral-800/80 px-4 py-2 text-gray-200 hover:bg-neutral-700/80" onClick={onSaved}>Abbrechen</button>
        <button type="submit" className="inline-flex items-center gap-2 rounded-2xl bg-violet-600 px-4 py-2 font-medium text-white shadow-lg shadow-violet-700/30 hover:brightness-110">Speichern</button>
      </div>
    </form>
  );
}
...
export default function MonthCalendar(){
  ...
  function DaysGrid(){
    const year=current.getFullYear(); const month=current.getMonth(); const first=new Date(year,month,1);
    const daysInMonth=new Date(year,month+1,0).getDate(); const startDow=(first.getDay()+6)%7;
      ...
      items.push(
        <div key={d} className={`relative h-16 sm:h-20 md:h-24 rounded-2xl border border-white/10 bg-neutral-900/80 transition-colors hover:bg-neutral-800/80 ${colStart}`}>
          ...
          <div className="absolute right-2 top-2 flex gap-1">
            {dayEvents.slice(0,4).map((ev,idx)=>(<span key={ev.id+idx} className="h-2 w-2 rounded-full" style={{background: TYPE_COLORS[ev.type]}}/>))}
          </div>
    ...
    <div className="text-gray-100">
      ...
      <div className="mt-4 mx-5 flex items-center justify-between rounded-2xl border border-white/10 bg-neutral-950/60 px-5 py-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="rounded-xl border border-white/10 bg-neutral-800/80 p-2 text-gray-300 hover:bg-neutral-700/80" onClick={()=>setCurrent(addMonth(current,1))}><ChevronRight size={18}/></button>
        </div>
      </div>
          ...
          <div className="max-h-[55vh] space-y-2 overflow-auto pr-2">
            {(listOpenFor ? (mapped[listOpenFor] || []) : []).map(ev => (
              <div key={ev.id} className="flex items-start gap-3 rounded-2xl border border-white/10 bg-neutral-800/80 p-3">
                ...
                <div>
                  <div className="font-medium">{ev.title} <span className="text-sm text-gray-300">• {ev.type}</span></div>
